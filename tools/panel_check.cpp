...
//
//   g++ -std=c++20 -Iexternal/reshade tools/panel_check.cpp src/ui/*.cpp src/ui/*/*.cpp -o panel_check
...
imgui_function_table Recorder()
{
    ...
    t.TextUnformatted = [](const char *s, const char *) { g_drawn.push_back(std::string("t:") + s); };
    ...
    t.SliderFloat = [](const char *l, float *, float, float, const char *, ImGuiSliderFlags) { return Hit(l, "w:"); };
    t.SliderInt = [](const char *l, int *, int, int, const char *, ImGuiSliderFlags) { return Hit(l, "w:"); };
    t.TreeNode = [](const char *l) {
        Hit(l, "h:");
        return (std::string(l) != "More settings" && std::string(l) != "Mais ajustes") || g_openMore;
    };
    t.TreePop = [] {};
    t.CollapsingHeader = [](const char *l, ImGuiTreeNodeFlags) { Hit(l, "h:"); return true; };
    t.CollapsingHeader2 = [](const char *l, bool *, ImGuiTreeNodeFlags) { Hit(l, "h:"); return true; };
...
std::set<std::string> Draw(ui::PanelSettings &s, const ui::PanelStatus &st, ui::PanelActions *out = nullptr,
                           bool withText = false)
{
    g_drawn.clear();
...
void CascadeOneByOne()
{
    ...
    for (int i = 0; i < ui::kOptCount; ++i)
    {
        ...
        CHECK(Draw(s, st).count(hdr[row.group]), "'%s' ticked, but its header %s is not drawn", row.en,
              hdr[row.group]);
    }
    for (int i = 0; i < ui::kOptCount; ++i)
        for (int j = i + 1; j < ui::kOptCount; ++j)
            for (const std::string &w : widgets[i])
                CHECK(!widgets[j].count(w), "'%s' and '%s' both draw %s", ui::kOpts[i].en, ui::kOpts[j].en,
                      w.c_str());

    // Nothing ticked: no Guides or Engine header at all.
    s = Base();
    const std::set<std::string> none = Draw(s, st);
    CHECK(!none.count("h:Guides") && !none.count("h:Engine"), "Guides/Engine header drawn with nothing ticked");
}
...
void CascadeRows()
{
    ...
    for (int i = 0; i < ui::kOptCount; ++i)
    {
        ...
        Draw(s, st);
        CHECK(s.optional == ui::kOpts[i].bit, "clicking '%s' left optional=0x%x", ui::kOpts[i].en, s.optional);
    }
    ui::PanelSettings s = Base();
    g_click = { "All" };
    Draw(s, st);
    ...
    CHECK(s.optional == (ui::kOptAll & ~ui::kOptFeed), "All on the 32-bit route gave 0x%x", s.optional);
    g_openMore = false;
    g_click.clear();
}
...
void Language()
{
    ...
    const std::set<std::string> allowed { "h:Debug", "h:Experimental", "w:Temporal", "w:Tonemap", "w:Passes",
                                          "w:Ctrl+END", "w:1/32", "t:(?)", "t:Debug", "t:Performance" };
...
void Actions()
{
    struct { const char *label; ui::PanelAction action; uint32_t opt; } cases[] {
    ...
    Draw(s, st, &a);
    g_click.clear();
    CHECK(s.scale == 1.0f && a.Has(ui::PanelAction::LiftScaleCap), "releasing Scale unchanged: scale=%.2f", s.scale);
}
...
{
    const ui::PanelStatus st = Status64();
    struct { uint32_t bit; const char *widget; void (*unmet)(ui::PanelSettings &); } cases[] {
    ...
    CHECK(!seen.count("w:Flow gate") && !seen.count("w:Flow accept"), "flow knobs drawn over game motion");
}
...
void Factory()
{
    ...
    CHECK(r.enabled == 1 && r.startOn == 1 && r.toggleKey == 0x23 && r.toggleMods == 1 && r.disableOnAltTab == 1 &&
              r.language == 1 && r.optional == cur.optional,
          "Factory Defaults lost a preference");
    CHECK(r.scale == 1.0f && r.passes == 1, "Factory Defaults kept tuning: scale %.2f passes %u", r.scale, r.passes);
}
...
void Route32()
{
    ...
    ui::PanelSettings s = Base();
    const std::set<std::string> seen = Draw(s, st, nullptr, true);
    CHECK(seen.count("t:protocol 3") && seen.count("t:passes 1"), "route diagnostics missing under Debug");
